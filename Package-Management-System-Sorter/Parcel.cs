 ...
 * 29 November 2016
 * CIS 200-01
 * New functionality to have an ascending natural sort order by cost
*/
...
{
    // Precondition:  None
    // Postcondition: When this < p, method returns negative #
    //                When this == p, method returns zero
    //                When this > p, method returns positive #
    public int CompareTo(Parcel p)
    {
        if (p == null)
            return 1;

        return this.CalcCost().CompareTo(p.CalcCost());
    }
